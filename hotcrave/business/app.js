...
function formatDate(value) {
  ...
  return new Intl.DateTimeFormat(businessLang === "en" ? "en-US" : "es-AR", { dateStyle: "short", timeStyle: "short" }).format(date);
}
...
function hotEventsView(message = "", error = false) {
  ...
  const eventCards = hotEvents.length
    ? hotEvents.slice().sort((a, b) => new Date(b.createdAt || b.availableAt || 0) - new Date(a.createdAt || a.availableAt || 0)).map((event) => {
        ...
        return `<article class="event-card"><div class="event-card-main"><div class="event-title-row"><h3>${escapeHtml(product?.name || "Producto")}</h3><span class="event-status ${statusClass(status)}">${escapeHtml(statusLabel(status))}</span></div><p class="muted">${businessLang === "en" ? "Available:" : "Disponible:"} ${escapeHtml(formatDate(event.availableAt))}</p><p class="event-expiry">${businessLang === "en" ? "Expires:" : "Vence:"} ${escapeHtml(formatDate(event.expiresAt))}</p></div>${canSoldOut ? `<button class="secondary sold-out" data-event-id="${escapeHtml(event.hotEventId)}">Marcar agotado</button>` : ""}</article>`;
      }).join("")
  ...
  const publishForm = isManager()
    ? `<section class="publish-card"><div><p class="eyebrow">NUEVO AVISO</p><h2>${businessLang === "en" ? "Freshly made food" : "Comida recién hecha"}</h2><p class="muted">${businessLang === "en" ? "Choose a product and when you want to notify customers." : "Elegí un producto y cuándo querés avisar."}</p></div><form id="hot-event-form"><label>${businessLang === "en" ? "Product" : "Producto"}<select name="productId" required ${publishableProducts.length ? "" : "disabled"}>${publishableProducts.length ? `<option value="">${businessLang === "en" ? "Select a product" : "Seleccioná un producto"}</option>${options}` : `<option>${businessLang === "en" ? "No products available to publish" : "No hay productos disponibles para publicar"}</option>`}</select></label>${!premium && products.some((product) => product.type === "CUSTOM") ? '<p class="muted small">Los productos personalizados requieren Premium. Tus productos guardados seguirán disponibles cuando vuelvas a activar Premium.</p>' : ""}<fieldset><legend>${businessLang === "en" ? "Initial status" : "Estado inicial"}</legend><label class="radio-option"><input type="radio" name="status" value="AVAILABLE_NOW" checked> ${businessLang === "en" ? "Available now" : "Disponible ahora"}</label><label class="radio-option"><input type="radio" name="status" value="READY_IN_15_MIN"> ${businessLang === "en" ? "Ready in 15 minutes" : "Listo en 15 minutos"}</label><label class="radio-option"><input type="radio" name="status" value="READY_IN_30_MIN"> ${businessLang === "en" ? "Ready in 30 minutes" : "Listo en 30 minutos"}</label></fieldset><button class="primary" type="submit" ${publishableProducts.length ? "" : "disabled"}>${businessLang === "en" ? "Publish Hot Event" : "Publicar Hot Event"}</button></form></section>`
    : `<section class="info-card"><strong>Solo los responsables del negocio pueden publicar Hot Events.</strong><p class="muted">${businessLang === "en" ? "Your current role is" : "Tu rol actual es"} ${escapeHtml(businessContext.membership?.role || (businessLang === "en" ? "Member" : "Miembro"))}.</p></section>`;

  render(shell(`${pageHeading(businessLang === "en" ? "PRIORITY" : "PRIORIDAD", "Hot Events", businessLang === "en" ? active + " of " + limit + " active. Publish quickly when your food is freshly made." : active + " de " + limit + " activos. Publicá rápido cuando tengas comida recién hecha.")}${message ? `<p class="${error ? "error" : "success"}" role="status">${escapeHtml(message)}</p>` : ""}${publishForm}<section class="section-heading compact"><p class="eyebrow">HISTORIAL</p><h2>Tus Hot Events</h2></section><div class="event-list">${eventCards}</div>`));
  bindShell();
...
function dashboardView() {
  ...
  const limit = business?.plan === "PREMIUM" ? 10 : 1;
  render(shell(`${pageHeading("RESUMEN", "Tu negocio", "Accedé rápidamente a todas las herramientas de Calentitos.")}<section class="hero-card"><div><p class="eyebrow">ESTADO</p><h2>${escapeHtml(business?.name || "Tu negocio")}</h2><p class="muted">${businessLang === "en" ? "Plan" : "Plan"} ${escapeHtml(business?.plan || "FREE")} · ${escapeHtml(businessContext.membership?.role || (businessLang === "en" ? "Member" : "Miembro"))}</p></div><button class="primary" id="open-hot-events">Publicar Hot Event</button></section><div class="stats-grid"><button class="stat-card stat-action" data-view="hot-events"><span>♨</span><strong>${active} / ${limit}</strong><p>${businessLang === "en" ? "Active Hot Events" : "Hot Events activos"}</p></button><button class="stat-card stat-action" data-view="products"><span>▦</span><strong>${products.length}</strong><p>${businessLang === "en" ? "Active products" : "Productos activos"}</p></button><button class="stat-card stat-action" data-view="qr"><span>⌁</span><strong>QR</strong><p>${businessLang === "en" ? "Share your business" : "Compartí tu negocio"}</p></button><button class="stat-card stat-action" data-view="profile"><span>◉</span><strong>Perfil</strong><p>${businessLang === "en" ? "Public information" : "Información pública"}</p></button></div>`));
  bindShell();
...
const BUSINESS_I18N = {
  ...
  'Alcanzaste el límite de productos de tu plan.':'You reached your plan’s product limit.','Los productos personalizados requieren HotCrave Premium.':'Custom products require HotCrave Premium.',
  'Las categorías personalizadas requieren HotCrave Premium.':'Custom categories require HotCrave Premium.','Alcanzaste el límite de Hot Events activos de tu plan.':'You reached your plan’s active Hot Events limit.',
